...
-- Migration: 20260828000000_initial_schema
...
-- Insert default settings (from Instagram data)
INSERT INTO settings (key, value) VALUES
  ('business_name', '"Menes Coffee & Eatery"'),
  ('address', '"Jl. Kartini, No. 24, Padang Pasir, Kota Padang, Sumatera Barat"'),
  ('phone', '""'),
  ('email', '""'),
  ...
  ('opening_hours', '{"monday": {"open": "09:00", "close": "03:00", "closed": false}, "tuesday": {"open": "09:00", "close": "03:00", "closed": false}, "wednesday": {"open": "09:00", "close": "03:00", "closed": false}, "thursday": {"open": "09:00", "close": "03:00", "closed": false}, "friday": {"open": "09:00", "close": "03:00", "closed": false}, "saturday": {"open": "09:00", "close": "03:00", "closed": false}, "sunday": {"open": "09:00", "close": "03:00", "closed": false}}')
...
CREATE POLICY "settings_admin_write" ON settings FOR ALL USING (
  EXISTS (SELECT 1 FROM admin_users WHERE auth_uid = auth.uid() AND role = 'superadmin')
);
...
SECURITY DEFINER
AS $$
  SELECT EXISTS (SELECT 1 FROM admin_users WHERE auth_uid = auth.uid() AND role = 'superadmin');
$$;
